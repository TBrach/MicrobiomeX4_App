...
# --
######################################
### FUNCTION: compare_beta_div_distances_simple
#######################################

# The idea was to make a simpler version of direct distance comparison more like the meandist function of vegan
# It gives you only one plot including all the with and between group distances defined by the group_var in your physeq object

compare_beta_div_distances_simple <- function(dist, physeq, group_var, jitter_up_to = 100, ggpubr = TRUE, test = "t.test", symnum.args = list(cutpoints = c(0, 1e-04, 0.001, 0.01, 0.05, 1), symbols = c("****", "***", "**", "*", "ns")),
                                                    p.adjust.method = "BH", hide.ns = FALSE) {
        ...
        if (length(dist) != nsamples(physeq)*(nsamples(physeq)-1)/2) {
                stop("The number of samples in the pruned physeq does not fit to the number of distances in dist_list. Check and come back.")
        }
        
        DistMat <- as(dist, "matrix")
        
        # - Transform the lower triangel of Distmatrix into a data frame -
        rowCol <- expand.grid(rownames(DistMat), colnames(DistMat))
        labs <- rowCol[as.vector(lower.tri(DistMat, diag=F)),]
        df <- cbind(labs, DistMat[lower.tri(DistMat, diag=F)])
        colnames(df) <- c("Row","Col","Distance")
        ...
        df$Row_Group <- samdf$Group[match(df$Row, samdf$Sample)]
        df$Col_Group <- samdf$Group[match(df$Col, samdf$Sample)]
        # NB: in the following you directly order the GroupToGroup based on the levels in samdf$Group
        df$GroupToGroup <- apply(dplyr::select(df, Row_Group, Col_Group), 1, function(x){
                paste(x[order(match(x, levels(samdf$Group)))], collapse = " to ")
        })
        # --
        ...
        # - order the GroupToGroup vector based on group_fac levels -
        Combis <- expand.grid(levels(group_fac), levels(group_fac))
        Levels <- apply(Combis, 1, function(x){paste(x[2], x[1], sep = " to ")})
        Levels <- Levels[Levels %in% unique(df$GroupToGroup)]
        df$GroupToGroup <- factor(df$GroupToGroup, levels = Levels, ordered = TRUE)
        # --
        ...
        if (ggpubr){
                Tr <- Tr + stat_compare_means(ref.group = Levels[1], label = "p.signif", method = test, hide.ns = hide.ns)
        }
...
# --
######################################
### FUNCTION: compare_beta_div_distances_intermediate
#######################################

# The idea was to make a simpler version of direct distance comparison more like the meandist function of vegan
# I think this function most closely results in a visual representation of the meandist matrix outcome
# It has duplicates (as the meandist matrix is a mirror matrix) but gives a nice overview of the beta diversity distances for each group compared to all others
# You don't like the duplicates use compare_beta_div_distances_simple 
# Here you get a faceted plot, for each level in group_var one, comparing that level to all other levels
# while in compare_beta_div_distances_directly_new you get an individual plot for each pairwise comparison, I would call that the complicated
# version

compare_beta_div_distances_intermediate <- function(dist, physeq, group_var, jitter_up_to = 100, ggpubr = TRUE, test = "t.test", symnum.args = list(cutpoints = c(0, 1e-04, 0.001, 0.01, 0.05, 1), symbols = c("****", "***", "**", "*", "ns")),
                                              p.adjust.method = "BH", hide.ns = FALSE) {
