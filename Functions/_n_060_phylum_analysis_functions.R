...
plot_sample_bars <- function(physeq, x = "Sample", y = "Abundance", group_var, color_levels, fill = NULL,
        ...
        # order fill levels according to abundance over all samples or by names of col_vectors
        ...
        if (order_by_firmicutes) {
                mdf_firmicutes <- dplyr::filter(mdf, Phylum == "Firmicutes") %>% arrange(!! sym(group_var), Abundance)
...
plot_taxa_ratios_AllLevels <- function(physeq, group_var, color_levels, tax_names = NULL,
        ...
        TbT_DF_l$Group <- factor(TbT_DF_l$Group, levels = group_var_levels, ordered = F)
        # --
        ...
        pVals <- ggpubr::compare_means(formula = Ratio ~ Group, data = TbT_DF_l, group.by = "Taxon", ref.group = Ref, method = test, p.adjust.method = p_adjust_method, symnum.args = symnum.args)
        ...
        pValsLog <- ggpubr::compare_means(formula = RatioLog10 ~ Group, data = TbT_DF_l, group.by = "Taxon", ref.group = Ref, method = test, p.adjust.method = p_adjust_method, symnum.args = symnum.args)
        ...
        if (is.null(Ref)){
                ...
                Tr1 <- Tr1 + ggpubr::stat_compare_means(ref.group = Ref, label = "p.signif", method = test, hide.ns = hide.ns)
        }
        ...
        if (is.null(Ref)){
                ...
                Tr3 <- Tr3 + ggpubr::stat_compare_means(ref.group = Ref, label = "p.signif", method = test, hide.ns = hide.ns)
        }
...
# --
####################################
## plot_taxa_ratio_to_continuous_variable 
###################################
# if cont_variable is NULL you plot the sample_sums!


plot_taxa_ratio_to_continuous_variables <- function(physeq, cont_variable = NULL, linearFits = TRUE, group_var = NULL, color_levels = NULL, tax_names = NULL,
                                       taxa_nom = "Firmicutes", taxa_den = NULL, tax_order = NULL) {
        ...
        # - add the sample_data  -
        LookUpDF <- as(sample_data(ps), "data.frame")
        LookUpDF$Sample <- rownames(LookUpDF)
        LookUpDF$Sample_Sum <- sample_sums(ps)
        
        TbT_DF_l <- merge(TbT_DF_l, LookUpDF, by = "Sample", all = TRUE)
        
        if (!is.null(group_var_levels)){
                TbT_DF_l[[group_var]] <- factor(TbT_DF_l[[group_var]], levels = group_var_levels, ordered = T)
        }
        ...
        # - define or test the continuous variable -
        if (is.null(cont_variable)){
                cont_variable <- "Sample_Sum"
        } else {
                
                if(! cont_variable %in% colnames(sample_data(physeq))) {
                        stop("The given cont_variable is not a variable in the sample data of the loaded phyloseq object.")
                }
        }
        # --
        ...
        if (!is.null(group_var)){
                ...
                Tr <- ggplot(TbT_DF_l, aes_string(x = cont_variable, y = "Ratio"))
                Tr <- Tr +
                        geom_point(aes_string(col = group_var), size = 1, alpha = 0.8) +
                        # scale_color_manual(values = c(color_lookup$color[i], color_lookup$color[j])) +
                        scale_color_manual(values = color_levels) +
                        facet_wrap(~ Taxon, scales = "free_y") +
                        ylab(paste("abundance ratio of", taxa_nom, "to stated taxon")) +
        ...
        # - added used dplyr to get a linear model for each taxon -
        # https://stackoverflow.com/questions/1169539/linear-regression-and-group-by-in-r
        
        if (linearFits){
                
                formulaa <- as.formula(paste("Ratio ~ ", cont_variable, sep = ""))
                
                Models <- TbT_DF_l %>% group_by(Taxon) %>% do(model = lm(formulaa, data = .))
                
                pVal_df <- t(sapply(Models$model, function(fit){data.frame(p_val = summary(fit)$coefficients[2,4], r_sqared = summary(fit)$r.squared)}))
                
                pVal_df <- cbind(Taxon = as.character(Models$Taxon), pVal_df)
        ...
        list(Tr = Tr, pVal_df = pVal_df, Models = Models)
