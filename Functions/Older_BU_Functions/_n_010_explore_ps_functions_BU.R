...
plot_sizeFactors <- function(physeq, SFs, group_var, color_levels, shape, Ref = NULL, test = "t.test", p_adjust_method = "fdr",
        ...
        if (!is.null(group_var)){
                ...
                if (is.null(Ref)){
                        ...
                        pVals <- compare_means(formula = formulaa, data = DF, ref.group = Ref, method = test, p.adjust.method = p_adjust_method, symnum.args = symnum.args)
        ...
        if (!is.null(group_var)){
                ...
                if (is.null(Ref)){
                        ...
                        pVals2 <- compare_means(formula = formulaa, data = DF, method = test, p.adjust.method = p_adjust_method, symnum.args = symnum.args)
                        ...
                        pVals2 <- compare_means(formula = formulaa, data = DF, ref.group = Ref, method = test, p.adjust.method = p_adjust_method, symnum.args = symnum.args)
