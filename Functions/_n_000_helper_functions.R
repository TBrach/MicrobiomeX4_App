...
#######################################
# why is "1 pL" the default? Because I think it corresponds best to the volume of a (yeast) cell.
# 1 L is 10 cm * 3. 
# 1 mL is then 1 cm * 3
# 1 µL is 1 mm * 3
# 1 nL is 100 µm * 3
# 1 pL is 10 µm * 3 and that I think is pretty close to a yeast cell
...
get_otu_table <- function(ps, taxa_as_rows = TRUE){
        otu <- as(otu_table(ps), "matrix")
        if (taxa_as_rows & !taxa_are_rows(ps)){
                otu <- t(otu)
        }
