...
boxplot_sampleSums <- function(physeq, group_var, color_levels, shape, test = "t.test", Ref = NULL, p_adjust_method = "fdr",
        ...
        DF2 <- DF
        DF2[[group_var]] <- factor(DF2[[group_var]], ordered = FALSE)
        ...
        if (is.null(Ref)){
                ...
                pVals <- compare_means(formula = formulaa, data = DF2, method = test, ref.group = Ref, p.adjust.method = p_adjust_method, symnum.args = symnum.args)
...
calc_alphadiv <- function(physeq, measures = c("Observed", "Shannon"), group_var, compare) {
...
# --
#######################################
### calc_linear_fitlists
#######################################

calc_linear_fitlists <- function(DF_alph, measures = c("Observed", "Shannon")) {
        
        fitlist_FR <- list()
        fitlist_Total <- list()
        for (i in 1:length(measures)){
                formulaa <- paste0(measures[i], " ~ Total")
                fit <- lm(formula = formulaa, data = DF_alpha_ref)
                fitlist_Total[[i]] <- fit
                formulaa <- paste0(measures[i], " ~ FilteredReads")
                fit <- lm(formula = formulaa, data = DF_alpha_ref)
                fitlist_FR[[i]] <- fit
        }
        
        names(fitlist_Total) <- measures
        names(fitlist_FR) <- measures
        
        list(fitlist_Total = fitlist_Total, fitlist_FR = fitlist_FR)
}
# --
...
calc_pVals_alphdiv <- function(DF_alpha, measures, group_var, compare, test = "t.test", Ref = NULL, 
                               symnum.args = list(cutpoints = c(0, 1e-04, 0.001, 0.01, 0.05, 1), symbols = c("****", "***", "**", "*", "ns")),
        ...
        for (i in 1:length(y_columns)){
                ...
                ttestList[[i]] <- ggpubr::compare_means(formula = comparison, ref.group = Ref, data = DF_alpha, method = test, p.adjust.method = p.adjust.method, symnum.args = symnum.args, paired = paired)
        }
...
boxplots_alphdiv <- function(DF_alpha, measures, group_var, shape, color_levels, Ref = NULL, test = "t.test", symnum.args = list(cutpoints = c(0, 1e-04, 0.001, 0.01, 0.05, 1), symbols = c("****", "***", "**", "*", "ns")), hide.ns = FALSE){
        ...
        for (i in 1:length(y_columns)){
                ...
                if (is.null(Ref)){
                        ...
                        Tr <- Tr + stat_compare_means(ref.group = Ref, method = test, label = "p.signif", symnum.args = symnum.args, hide.ns = hide.ns)
                }
...
boxplots_alphdiv2 <- function(DF_alpha, PlotVariables, group_var, shape, color_levels,
                              facet_x = NULL, facet_y = NULL, Ref = NULL, test = "t.test", symnum.args = list(cutpoints = c(0, 1e-04, 0.001, 0.01, 0.05, 1), symbols = c("****", "***", "**", "*", "ns")), hide.ns = FALSE){
        ...
        for (i in 1:length(PlotVariables)){
                aes_map <- aes_string(x = group_var, y = PlotVariables[i], color = group_var)
                Tr <-  ggplot(DF_alpha, aes_map) + 
                        geom_boxplot(na.rm = TRUE, outlier.color = NA) +
                        geom_jitter(aes_string(shape = shape), width = .2, height = 0, alpha = 0.65) +
                        xlab("") +
                ...
                if (!is.null(facet_x) || !is.null(facet_y)){
                        formulaa <- as.formula(paste0(facet_x, " ~ ", facet_y))
                        Tr <- Tr + facet_grid(formulaa)
                }
                if (is.null(Ref)){
                        Tr <- Tr + stat_compare_means(comparisons = comparisonList, method = test, label = "p.signif", symnum.args = symnum.args, hide.ns = hide.ns) # "p.signif"
                } else {
                ...
                boxplotList[[i]] <- Tr
                names(boxplotList)[i] <- PlotVariables[i]
        }
        boxplotList
}
# --
...
lmPlots_alphdiv2 <- function(DF_alpha, lm_fitlist, measures, xval = "Total", group_var, shape, color_levels, test = "t.test", alpha = 1){
        ...
        lm_plotList <- list()
        for (i in 1:length(measures)){
                aes_map <- aes_string(x = xval, y = measures[i], color = group_var, shape = shape)
                Tr <-  ggplot(DF_alpha, aes_map) + 
                        geom_point(na.rm = TRUE, alpha = alpha) +
                        # xlab("total counts (sample_sums())") +
                        scale_color_manual("", values = color_levels) +
                ...
                DF_alpha$Predicted <- predict(fit, newdata = DF_alpha)
                
                Tr <- Tr + geom_line(data = DF_alpha, aes_string(x = xval, y = "Predicted"), col = "blue", lwd = 1)
