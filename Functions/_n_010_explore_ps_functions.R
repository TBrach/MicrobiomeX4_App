...
calc_SFs <- function(physeq, zeros.count = FALSE, percentile = 50)  {
        ...
        if (taxa_are_rows(physeq)) {
                ...
                SFs <- SFs/exp(mean(log(SFs), na.rm = TRUE))
        } else {
        ...
        if (sum(is.na(SFs)) > 0 || min(SFs) == 0) { warning("in at least one sample the Size Factor was 0 or NA") }
...
plot_sample_bars_compare <- function(physeq, physeq2, x = "Sample", y = "Abundance", group_var = NULL, color_levels = NULL, fill = NULL,
                                     color_sample_names = FALSE, col_vec = NULL, order_by_raw_counts = TRUE){
        ...
        if (!is.null(group_var)){
                ...
                # in case color_levels does not cover all samples
        ...
        mdf[, fill][is.na(mdf[, fill])] <- "NA" # NB: pools all NA
        if (is.null(col_vec)){
                sums <- group_by_at(mdf, fill) %>% summarise(sum_abundance = sum(Abundance)) %>% arrange(sum_abundance)
                mdf[, fill] <- factor(mdf[, fill], levels = as.character(sums[[1]]), ordered = TRUE)
        } else {
                mdf[, fill] <- factor(mdf[, fill], levels = rev(names(col_vec)), ordered = TRUE)
        }
        ...
        if (is.null(col_vec)){
                ...
                mdf2[, fill] <- factor(mdf2[, fill], levels = rev(names(col_vec)), ordered = TRUE)
        }
        # --
        ...
        if (!is.null(group_var)){
                ...
                if (order_by_raw_counts) {
                        rawSampleSizes <- data.frame("Sample" = sample_names(physeq), "Total" = sample_sums(physeq))
                        rawSampleSizes <- arrange(rawSampleSizes, Total)
                        mdf$Sample <- factor(mdf$Sample, levels = rawSampleSizes$Sample, ordered = TRUE)
                        
                } else {
                        mdf$Sample <- factor(mdf$Sample, levels = sample_names(physeq), ordered = TRUE)
        ...
        if (color_sample_names){
                Tr <- Tr +
                        theme(axis.text.x = element_text(colour = colxaxis))
        }
...
plot_sizeFactors <- function(physeq, SFs, group_var, color_levels, shape, Ref = NULL, test = "t.test", p_adjust_method = "fdr",
        ...
        if (!is.null(group_var)){
                ...
                DF2 <- DF
                DF2[[group_var]] <- factor(DF2[[group_var]], ordered = FALSE) # needed for compare_means that uses relevel that is not allowed on ordered factors
                ...
                if (is.null(Ref)){
                        ...
                        pVals <- compare_means(formula = formulaa, data = DF2, method = test, p.adjust.method = p_adjust_method, symnum.args = symnum.args)
                        ...
                        pVals <- compare_means(formula = formulaa, data = DF2, ref.group = Ref, method = test, p.adjust.method = p_adjust_method, symnum.args = symnum.args)
        ...
        DF2$total_count <- sample_sums(physeq)
        ...
        # - add also a comparison just of the original library sizes -
        
        if (!is.null(group_var)){
                
                Tr2 <-  ggplot(DF, aes_string(x = group_var, y = "total_count", color = group_var)) 
                Tr2 <- Tr2 + 
                        geom_boxplot(outlier.color = NA) +
                        geom_jitter(aes_string(shape = shape), width = .2, height = 0, alpha = 0.65) +
                        xlab("") +
                        ylab("total count (sample_sums)") +
                        scale_color_manual("", values = color_levels) +
                        theme_bw()
                if(is.null(shape)){
                ...
                if (is.null(Ref)){
                        ...
                        pVals2 <- compare_means(formula = formulaa, data = DF2, method = test, p.adjust.method = p_adjust_method, symnum.args = symnum.args)
                        ...
                        pVals2 <- compare_means(formula = formulaa, data = DF2, ref.group = Ref, method = test, p.adjust.method = p_adjust_method, symnum.args = symnum.args)
        ...
        list(pVals = pVals, Tr = Tr, Tr1 = Tr1, Tr_Total = Tr2, pVals_Total = pVals2)
}
# --
...
illustrate_ASV_Overlap <- function(seqtab1, seqtab2, color_levels, seqtab.names = NULL){
        ...
        if (!is.null(seqtab.names)){
                one <- seqtab.names[1]
                two <- seqtab.names[2]
                one.specific <- paste0(one, "_specific")
                two.specific <- paste0(two, "_specific")
        ...
        DF <- data.frame(Seqtab = c(rep(one,2), rep(two, 2), rep("Merged", 3)), From = c(one.specific, "Overlap", two.specific, "Overlap",
                                                                                         one.specific, two.specific, "Overlap"),
                         No_ASV = c(Seqtab1_specific, Overlap, Seqtab2_specific, Overlap, Seqtab1_specific, Seqtab2_specific,  Overlap))
        
        
        
        DF$Seqtab <- factor(DF$Seqtab, levels = c(one, two, "Merged"), ordered = T)
        DF$From <- factor(DF$From, levels = c(one.specific, two.specific, "Overlap"), ordered = T)
        ...
        DF1 <- data.frame(From = c(one.specific, two.specific, "Overlap", "Total"), No_ASV = c(Seqtab1_specific, Seqtab2_specific, Overlap, sum(Seqtab1_specific, Seqtab2_specific, Overlap)))
        DF1$From <- factor(DF1$From, levels = c(one.specific, two.specific, "Overlap", "Total"), ordered = TRUE)
